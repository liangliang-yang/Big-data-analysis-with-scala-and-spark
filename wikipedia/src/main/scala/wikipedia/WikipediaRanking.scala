...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)(
      (sum, article) => sum + isFound(article, lang),
      (sum1, sum2) => sum1 + sum2
    )
  }
...
//    https://github.com/andreformento/big-data-analysis-with-scala-and-spark/blob/master/03-programming-assignment-wikipedia/src/main/scala/wikipedia/WikipediaRanking.scala
...
//    https://github.com/andreformento/big-data-analysis-with-scala-and-spark/blob/master/03-programming-assignment-wikipedia/src/main/scala/wikipedia/WikipediaRanking.scala
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    makeIndex(langs, rdd).map(row => (row._1,row._2.size)).
                                      reduceByKey(_ + _).
                                      sortBy(_._2, false).
                                      collect().
                                      toList
